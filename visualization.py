...
def visualization(model, trans, sent):
  ...
  for layer in range(1, 6, 2):
    fig, axs = plt.subplots(1, 4, figsize=(len(sent.split(' ')), len(sent.split(' '))))
  ...
  for layer in range(1, 6, 2):
    fig, axs = plt.subplots(1, 4, figsize=(len(trans), len(trans)))
    ...
    fig, axs = plt.subplots(1, 4, figsize=(len(sent.split(' ')), len(trans)))
    for h in range(4):
